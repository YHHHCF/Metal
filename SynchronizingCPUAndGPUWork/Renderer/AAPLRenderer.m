...
@implementation AAPLRenderer
{
    ...
    // Can simultaneously work on 3 frames at any time
    dispatch_semaphore_t _inFlightSemaphore;

    // A series of buffers containing dynamically-updated vertices.
    // each buffer stores one frame (all triangles' vertices)
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    if(self)
    {
        ...
        // aliases rasterSampleCount property
        pipelineStateDescriptor.sampleCount = mtkView.sampleCount;
       
        // the app guarantees that it finishes writing data to a buffer instance
        // before it encodes a command that references the buffer instance
        // This order allows you to mark your buffer instances as immutable.
        // Metal can optimize the performance of immutable buffers, but not mutable buffers.
        // For best results, use immutable buffers as much as possible.
        ...
        // create MaxFramesInFlight buffers to hold MaxFramesInFlight frames
        // each frame has all the triangles
        for(NSUInteger bufferIndex = 0; bufferIndex < MaxFramesInFlight; bufferIndex++)
        {
            // buffers are not initialzed now, each buffer's capacity could hold
            // all triangles' vertices, and each buffer is shared between CPU and GPU
...
{
    NSLog(@"Size change: %f, %f\n", size.width, size.height);
    // Regenerate the triangles.
//    [self generateTriangles]; // why do we need this?
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    // This is needed because the number of buffers are limited
    // This decrement semaphore's counting by 1, if below 0 then wait until it becomes 0
    ...
    // addCompletedHandler registers a block of code that will be called immediately after GPU
    // finishes the command buffer
    [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> cb)
     {
         // cb is actually the argument passed into the block, it is the commandBuffer itself
         // increase semaphore by 1
         dispatch_semaphore_signal(block_semaphore);
         NSLog(@"Debug addCompletedHandler: %lf\n", cb.GPUEndTime -  cb.GPUStartTime);
     }];
